...
extern UINT8* mstpdata_get_esi_name_for_vlan(VLAN_ID vlan_id);
extern bool mstpdata_set_esi_for_vlan(VLAN_ID vlan_id, UINT8 *esi_name);
extern bool mstpdata_is_valid_esi_name(UINT8 *esi_name);
extern VLAN_ID mstpdata_get_esi_vlan(VLAN_ID current_vlan_id);
...
extern bool mstputil_is_operPt2pt_port(PORT_ID port_number);
...
extern bool mstputil_root_protect_timer_expired(MSTP_INDEX mstp_index, PORT_ID port_number);
extern void mstputil_clear_timer(UINT16 * timer);
extern bool mstputil_cist_root_protect_violation(PORT_ID port_number, MSTP_CIST_PORT *cist_port);
extern bool mstputil_msti_root_protect_violation(PORT_ID port_number, MSTP_MSTI_PORT *msti_port, MSTP_INDEX mstp_index);
extern bool mstputil_root_protect_validate(PORT_ID port_number, MSTP_BPDU *bpdu );
...
extern void mstpmgr_itc_configuration(void * req_ptr, void * resp_ptr);
...
extern void mstpmgr_process_ipc_msg(STP_IPC_MSG *msg, int len, struct sockaddr_un client_addr);
...
extern bool mstplib_is_mstp_configured();
